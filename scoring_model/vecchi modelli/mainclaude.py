...
if not claude_api_key:
    raise ValueError("Anthropic API key not found in .env.local file")

# Set up DSPY with new LM configuration
lm = dspy.LM('anthropic/claude-3-5-sonnet-latest', api_key=claude_api_key)
...
class AnalyzeRule(dspy.Signature):
    ...
    def __call__(self, interview_response, rule_id, rule_condition):
        # Further reduce token count by truncating more aggressively
        max_chars = 300  # Reduced further to stay within rate limits
        ...
        try:
            response = lm(messages=messages, temperature=0.1, max_tokens=150)  # Further reduced max_tokens
...
def analyze_interview(interview_response, scoring_rules):
    ...
    for category, rules in scoring_rules.items():
        for rule in rules:
            ...
            time.sleep(6)  # Increased to 2 second delay between calls
...
def calculate_final_score(scores, evaluations):
    ...
    with open("interview_evaluation_claude.txt", "w", encoding="utf-8") as file:
        file.write(output)
